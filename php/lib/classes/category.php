...
class Category
{
	...
	public $id = null;			// @var int The category ID from the database
	public $name = null;		// @var string Name of the category
	public $description = null;	// @var string A short description of the category
	...
	public static function getList($numRows=1000000, $order="name ASC") {
		$conn = new PDO( DB_DSN, DB_USERNAME, DB_PASSWORD );
		$sql = "SELECT SQL_CALC_FOUND_ROWS * FROM categories ORDER BY " . mysql_escape_string($order) . " LIMIT :numRows";
		$st = $conn->prepare($sql);
